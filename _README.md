...
- **スマート予約管理**: サービス時間に応じて予約可能な日付・時間帯を自動表示
...
2. **予約ページ** (`reservation.html`)
   ...
   - サービス選択（時間計算機能付き）
   - ペット・飼い主情報入力フォーム
   - 動的な予約可能状況表示（サービス時間に応じて利用不可な日付・時間を自動判定）
...
### 予約システムの特徴
- **スマート時間管理**: 選択したサービスの合計時間に基づいて、予約可能な時間枠を自動計算
- **リアルタイム更新**: サービスを追加・削除すると、カレンダーの予約可能状況が即座に更新
- **視覚的フィードバック**: 
  - ◯: 空きあり
  - △: 残りわずか
  - 満: 満席（ランダム表示）
...
### レスポンシブ対応
- 768px以下: タブレット用の調整（フォントサイズ、余白の削減）
- 480px以下: スマートフォン用の最適化（さらに小さいフォント、コンパクトな表示）
